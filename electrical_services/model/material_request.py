...
class MaterialRequest(models.Model):
    _name = 'electrical.material.request'
    _rec_name = 'name'
    ...
    site_reference = fields.Many2many('electrical.site.ref', string="Site Reference")
    ...
    pending_list = fields.Many2one('electrical.work.order', string="Pending List",
                                   help="Pending Order Lists to create Request", copy=False)
    material_bom_line = fields.One2many('electrical.material.request.line', 'material_request_id',
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('electrical.material.request' or _('New'))
    ...
    def action_send_email(self):
        ...
        ctx = {
            'default_model': 'electrical.material.request',
            'default_template_id': self.env.ref('electrical_services.email_template_electrical_material_request').id,
        }
        self.state = 'send'
        return {
    ...
    def generate_purchase_order(self):
        ...
        view_id = self.env.ref('electrical_services.view_purchase_order_wizard').id
        return {
            'name': _('Create Purchase Order'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'create.purchase',
            'views': [[view_id, 'form']],
            'target': 'new',
            'context': ctx,
        }
...
class WorkOrderBomLine(models.Model):
    ...
    _name = 'electrical.material.request.line'
    _description = 'Material Request Line'

    material_request_id = fields.Many2one('electrical.material.request', string="Material Request")
