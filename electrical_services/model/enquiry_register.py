...
class EnquiryRegister(models.Model):
    _name = 'electrical.enquiry.register'
    ...
    enquiry_site_reference = fields.Many2many('electrical.site.ref', string="Site Reference")
    enquiry_project_id = fields.Many2one('project.project', string="Project Name",
                                         default=lambda self: self.env.ref(
                                             'project_data.project_project_electrical').id)
    electrical_project = fields.Char(string='Project')
    ...
    enquiry_electrical_scope_ids = fields.One2many('enquiry.electrical.scope.line', 'enquiry_id')
    enquiry_electrical_parts_ids = fields.One2many('enquiry.electrical.parts.line', 'enquiry_parts_id')
    ...
    sale_order_id = fields.Many2one('sale.order', string='Sale Order Reference', domain=[('is_e_sale', '=', True)])
    ...
    def _onchange_sale_order_id(self):
        ...
        for order_line in self.sale_order_id.order_line:
            vals = {
                ...
                'enquiry_scope_code': order_line.product_id.default_code,
                'enquiry_scope_description': order_line.product_id.electrical_item_description,
            ...
            self.update({'enquiry_electrical_scope_ids': scope_list})

    def _compute_work_order_count(self):
        for enquiry_id in self:
            enquiry_id.work_order_count = enquiry_id.env['electrical.work.order'].search_count(
                [('enquiry_id', '=', enquiry_id.id)]) or 0
            enquiry_id.work_quotation_count = enquiry_id.env['electrical.quotation'].search_count(
                [('enquiry_id', '=', enquiry_id.id)]) or 0

    @api.depends('enquiry_electrical_scope_ids.enquiry_total')
    def _compute_amount(self):
        for scope in self:
            amount_untaxed = amount_tax = 0.0
            for line in scope.enquiry_electrical_scope_ids:
    ...
    def _onchange_enquiry_customer_id(self):
        amc_contract_id = self.env['amc.contract'].search(
    ...
    def submit_enquiry(self):
        if self.check_amc:
            ...
            self.state = 'submit'
        self.name = self.env['ir.sequence'].next_by_code('electrical.enquiry.register')

    @api.onchange('enquiry_electrical_scope_ids')
    @api.depends('enquiry_electrical_scope_ids')
    def _onchange_enquiry_electrical_scope_ids(self):
        """ Get values from electrical spare parts and assign to Bill of material """
        spare_parts_list = [(5, 0, 0)]
        for spare_parts_ids in self.enquiry_electrical_scope_ids:
            if spare_parts_ids:
                for spare_parts_id in spare_parts_ids:
                    for spares in spare_parts_id.enquiry_scope_id.electrical_spare_parts_ids:
                        spare_parts_vals = {
                        ...
                        if self.contract_id:
                            for spare in self.contract_id.amc_electrical_parts_ids:
                        ...
                        self.update({'enquiry_electrical_parts_ids': spare_parts_list})

    def add_attachments(self):
        self.ensure_one()
        res = self.env['ir.actions.act_window']._for_xml_id('base.action_attachment')
        res['domain'] = [('res_model', '=', 'electrical.enquiry.register'),
                         ('res_id', '=', self.id)]
        res['context'] = {'default_res_model': 'electrical.enquiry.register',
                          'default_res_model_name': 'Electrical Enquiry Register',
                          'default_res_id': self.id}
        return res

    def create_quotation(self):
        """ Create quotation from enquiry """
        electrical_quotation = self.env["electrical.quotation"]
        vals = {
            'electrical_quotation_date': fields.Date.today(),
            'electrical_customer_id': self.enquiry_customer_id.id,
            'electrical_customer_address': self.enquiry_customer_address,
            'electrical_site_ref': self.enquiry_site_reference.ids,
            'electrical_project_id': self.enquiry_project_id.id,
        ...
        quotation_id = electrical_quotation.create(vals)
        if quotation_id:
            if self.enquiry_electrical_scope_ids:
                for scope in self.enquiry_electrical_scope_ids:
                    scope_list = []
                    vals = (0, 0, {
            ...
            if self.enquiry_electrical_parts_ids:
                for parts in self.enquiry_electrical_parts_ids:
                    spare_parts_list = []
                    vals = (0, 0, {
        ...
        view_id = self.env.ref('electrical_services.view_electrical_quotation_form').id
        return {
            'view_mode': 'form',
            'res_model': 'electrical.quotation',
            'view_id': view_id,
    ...
    def work_order_approve(self):
        ...
        approved list"""
        if self.enquiry_electrical_scope_ids and self.enquiry_electrical_parts_ids:
            self.sudo().write({
                'state': 'approve'
            })
        else:
        ...
        if location_id and destination_id:
            ...
            work_order = self.env['electrical.work.order'].create({
                ...
                'electrical_work_scope_line_ids': [(0, 0, {
                    'electrical_work_scope_id': enquiry_electrical_scope_id.enquiry_scope_id.id,
                    'electrical_work_scope_code': enquiry_electrical_scope_id.enquiry_scope_code,
                    'electrical_work_scope_description': enquiry_electrical_scope_id.enquiry_scope_description,
                    'electrical_work_qty': enquiry_electrical_scope_id.enquiry_scope_qty,
                    'electrical_work_scope_price': enquiry_electrical_scope_id.enquiry_scope_price,
                    'electrical_work_tax': enquiry_electrical_scope_id.enquiry_tax.ids,
                    'electrical_work_tax_amount': enquiry_electrical_scope_id.enquiry_tax_amount,
                    'electrical_work_subtotal': enquiry_electrical_scope_id.enquiry_subtotal,
                    'electrical_work_total': enquiry_electrical_scope_id.enquiry_total,
                }) for enquiry_electrical_scope_id in self.enquiry_electrical_scope_ids],
                'electrical_work_spare_line_ids': [(0, 0, {
                    'picking_type_id': picking_type_id.id,
                    'picking_id': picking_id.id,
                    'name': enquiry_electrical_parts_id.enquiry_spare_parts_id.name,
                    'product_id': enquiry_electrical_parts_id.enquiry_spare_parts_id.id,
                    'product_uom_qty': enquiry_electrical_parts_id.enquiry_spare_parts_qty,
                    'price_unit': enquiry_electrical_parts_id.enquiry_spare_parts_unit_price,
                    'location_id': picking_type_id.default_location_src_id.id,
                    'product_uom': enquiry_electrical_parts_id.enquiry_spare_parts_id.uom_id.id,
                    'location_dest_id': enquiry_electrical_parts_id.enquiry_spare_parts_id.electrical_stock_location.id,
                }) for enquiry_electrical_parts_id in self.enquiry_electrical_parts_ids],
            })
            return {
                'name': 'Work Order',
                'type': 'ir.actions.act_window',
    ...
    def button_electrical_quotation(self):
...
class EnquiryScopeLine(models.Model):
    _name = 'enquiry.electrical.scope.line'
    _description = 'Enquiry Electrical Scope'

    enquiry_id = fields.Many2one('electrical.enquiry.register')
    enquiry_scope_id = fields.Many2one('product.product', string='Electrical Scope',
                                       domain=[('is_electrical', '=', True)])
    enquiry_scope_code = fields.Char(string='Item code', related='enquiry_scope_id.default_code')
    ...
    enquiry_scope_price = fields.Float(string='Amount', related='enquiry_scope_id.lst_price')
    enquiry_tax = fields.Many2many('account.tax', string='VAT')
    enquiry_tax_amount = fields.Float(string='VAT Amount', compute='_compute_enquiry_total')
    ...
    def _compute_enquiry_total(self):
        if self.enquiry_id.contract_id:
            for electrical_scope in self.enquiry_id.contract_id.amc_electrical_scope_ids:
                for scope in self:
                    if electrical_scope.amc_scope_id.id == scope.enquiry_scope_id.id:
                        scope.update(
                            {'enquiry_scope_price': electrical_scope.amc_scope_price})
...
class ElectricalPartsLine(models.Model):
    _name = 'enquiry.electrical.parts.line'
    _description = 'Enquiry Electrical Spare Parts'

    enquiry_parts_id = fields.Many2one('electrical.enquiry.register')
