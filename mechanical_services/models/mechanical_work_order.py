...
class MechanicalWorkOrder(models.Model):
    _name = 'mechanical.work.order'
    _rec_name = 'name'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = 'Mechanical Work Order'
    ...
    mechanical_technician_ids = fields.One2many('mechanical.technician.line', 'work_order_id')
    ...
    def _onchange_sale_order_reference(self):
        ...
        for word_order in self:
            word_order.sale_order_reference.m_work_order = word_order.id
    ...
    def _compute_check_dead_line(self):
        ...
        for work_order in self:
            project_task = work_order.env['project.task'].search([('mechanical_order_id', '=', work_order.id)], limit=1)
    ...
    def create(self, vals):
        if not vals.get('name') or vals['name'] == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code(
                'mechanical.work.order') or _('New')
        return super(MechanicalWorkOrder, self).create(vals)

    def _compute_count(self):
        for order in self:
            order.material_request_count = self.env[
                'mechanical.material.request'].search_count(
                [('pending_list', '=', order.id)])
            order.task_count = self.env['project.task'].search_count(
                [('mechanical_order_id', '=', order.id)])
            order.invoice_count = self.env['account.move'].search_count(
                [('mechanical_work_order_id', '=', order.id)])
            order.completion_certificate_count = self.env['mech.completion.certificate'].search_count(
    ...
    def action_view_material_request(self):
        ...
        return {
            ...
            'view_mode': 'tree,form',
            'res_model': 'mechanical.material.request',
    ...
    def action_view_completion_certificate(self):
        ...
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_mode': 'tree,form',
            'res_model': 'mech.completion.certificate',
    ...
    def action_view_invoice(self):
        ...
        return {
            ...
            'domain': [('mechanical_work_order_id', '=', self.id)],
            'target': 'current',
            'context': {'create': False}
        }
    ...
    def action_view_tasks(self):
        ...
        return {
            ...
            'domain': [('mechanical_order_id', '=', self.id)],
            'target': 'current',
            'context': {'create': False}
        }

    def check_availability(self):
        moves = self.mechanical_work_spare_line_ids.filtered(
            lambda x: x.state not in ('done', 'cancel'))._action_confirm()
        moves._action_assign()
        material_request = self.env['mechanical.material.request']
        material_request_line = self.env['mechanical.material.request.line']
        if any(line.state in ['confirmed', 'partially_available'] for line in
               self.mechanical_work_spare_line_ids):
            if self.material_request_count == 0:
                ...
                material_request_line.create({
                                             ...
                                             self.mechanical_work_spare_line_ids.filtered(
    ...
    def start_work(self):
        ...
        self.mechanical_order_picking_id.origin = self.name
        for line in self.mechanical_work_spare_line_ids:
            line.mechanical_order_move_id = self.id
            line.quantity_done = line.product_uom_qty
        self.mechanical_order_picking_id.button_validate()
        self.mechanical_order_picking_id.message_post_with_view(
            'mail.message_origin_link',
            values={'self': self.mechanical_order_picking_id, 'origin': self},
            subtype_id=self.env.ref('mail.mt_note').id)
        technician_list = []
        for technician_line in self.mechanical_technician_ids:
    ...
    def generate_completion_certificate(self):
        certificate = self.env['mech.completion.certificate']
        certificate_id = certificate.create({
            ...
            'enquiry_mechanical_scope_ids': [(0, 0, {
                'enquiry_scope_id': mechanical_scope.enquiry_scope_id.id,
                'enquiry_scope_code': mechanical_scope.enquiry_scope_code,
                'enquiry_scope_description': mechanical_scope.enquiry_scope_description,
                'enquiry_scope_qty': mechanical_scope.enquiry_scope_qty,
                'enquiry_scope_price': mechanical_scope.enquiry_scope_price,
                'enquiry_tax': mechanical_scope.enquiry_tax,
                'enquiry_tax_amount': mechanical_scope.enquiry_tax_amount,
                'enquiry_subtotal': mechanical_scope.enquiry_subtotal,
                'enquiry_total': mechanical_scope.enquiry_total
            })
                                             for mechanical_scope in
                                             self.enquiry_id.enquiry_mechanical_scope_ids],
            'enquiry_spare_parts_ids': [(0, 0, {
                ...
                'enquiry_spare_parts_unit_price': spare_part.enquiry_spare_parts_unit_price
                                        ...
                                        for spare_part in
                                        self.enquiry_id.enquiry_mechanical_parts_ids],
        })
        return {
            'name': 'Certificate',
            'type': 'ir.actions.act_window',
...
class EnquiryScopeLine(models.Model):
    _name = 'mech.work.scope.line'
    _description = 'Mechanical Work Scope'

    mechanical_work_id = fields.Many2one('mechanical.work.order')
    mechanical_work_scope_id = fields.Many2one('product.product',
                                               string='Mechanical Scope',
                                               domain=[('is_mechanical', '=',
                                                        True)])
    mechanical_work_scope_code = fields.Char(string='Item code')
    mechanical_work_scope_description = fields.Char(string='Item description')
    mechanical_work_scope_price = fields.Float(string='Amount')
    mechanical_work_qty = fields.Float(string='Quantity')
    mechanical_work_tax = fields.Many2many('account.tax', string='VAT')
    mechanical_work_tax_amount = fields.Float(string='VAT Amount')
    mechanical_work_subtotal = fields.Float(string='Subtotal')
    mechanical_work_total = fields.Float(string='Total')


class StockMove(models.Model):
    _inherit = 'stock.move'

    mechanical_order_move_id = fields.Many2one('mechanical.work.order',
                                               string="Order Reference")


class MechanicalTechnicianDetails(models.Model):
    _name = 'mechanical.technician.line'
